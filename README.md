...
If the segment you're attempting to parse has a func in it (call or declaration, this is not necessary for indirect calls), then you should first use the code provided in func_definitions_builder.js. This code outputs a JSON stringified object of function definition data to the func_definitions.txt that the parser uses in order to effectively handle functions. Just make sure you have input your watfile (or really only the necessary segments of it, but it's easier to just input the whole thing) into the watfile.txt, and run the builder. If the segment you're attempting to parse does not have a func in it, then this step is not necessary.
